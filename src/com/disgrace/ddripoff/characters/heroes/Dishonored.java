package com.disgrace.ddripoff.characters.heroes;

import java.util.ArrayList;

import javax.xml.bind.annotation.XmlRootElement;

import com.disgrace.ddripoff.spells.instance.DishonoredBloodyRedemption;
import com.disgrace.ddripoff.spells.instance.DishonoredChainedDeath;
import com.disgrace.ddripoff.spells.instance.DishonoredOpenWounds;
import com.disgrace.ddripoff.spells.instance.DishonoredSuffocate;
import com.disgrace.ddripoff.stats.Stat;
import com.disgrace.ddripoff.stats.StatEnumeration;
import com.disgrace.ddripoff.stats.StatWrapper;

@XmlRootElement
public class Dishonored extends Hero {

	public Dishonored() {
		name = "Dishonored";
		heroClass = HeroClass.DISHONORED;
		stats = new StatWrapper(new ArrayList<Stat>() {
			{
				add(new Stat(StatEnumeration.HEALTH, 21, 21));
				add(new Stat(StatEnumeration.DODGE, 7, 50));
				add(new Stat(StatEnumeration.PHYS_PROT, 0, 100));
				add(new Stat(StatEnumeration.DAMAGE, 10, 100));
				add(new Stat(StatEnumeration.SPEED, 6, 20));
				add(new Stat(StatEnumeration.ACC, 0, 100));
				add(new Stat(StatEnumeration.CRIT_CHANCE, 5, 100));

				add(new Stat(StatEnumeration.DEATH_RES, 66, 100));
				add(new Stat(StatEnumeration.MOVE_RES, 50, 220));
				add(new Stat(StatEnumeration.BLEED_RES, 50, 220));
				add(new Stat(StatEnumeration.BLIGHT_RES, 30, 220));
				add(new Stat(StatEnumeration.DEBUFF_RES, 20, 220));
				add(new Stat(StatEnumeration.STUN_RES, 50, 220));

				add(new Stat(StatEnumeration.RANGE_MOD, 0, 100));
				add(new Stat(StatEnumeration.MELEE_MOD, 0, 100));
				add(new Stat(StatEnumeration.PHYS_MOD, 0, 100));

				add(new Stat(StatEnumeration.MOVE_CHANCE, 0, 220));
				add(new Stat(StatEnumeration.BLIGHT_CHANCE, 0, 220));
				add(new Stat(StatEnumeration.BLEED_CHANCE, 0, 220));
				add(new Stat(StatEnumeration.STUN_CHANCE, 0, 220));
				add(new Stat(StatEnumeration.DEBUFF_CHANCE, 0, 220));
			}
		});
		allAbilities.add(new DishonoredBloodyRedemption());
		allAbilities.add(new DishonoredChainedDeath());
		allAbilities.add(new DishonoredOpenWounds());
		allAbilities.add(new DishonoredSuffocate());
		learnedAbilities.addAll(allAbilities);
	}

}
